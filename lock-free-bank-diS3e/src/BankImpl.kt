...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        while (true) {
            ...
            if (!account.invokeOperation()) {
                ...
                val newAccount = Account(account.amount - amount)
                if (accounts[index].compareAndSet(account, newAccount)) {
    ...
    private fun release(index: Int, op: Op) {
        ...
        if (account is AcquiredAccount && account.op === op) {
            ...
            accounts[index].compareAndSet(account, Account(account.newAmount))
        }
    }
